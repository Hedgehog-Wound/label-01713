...
- 三种生成模式：单餐（可选早餐/午餐/晚餐）/ 全天 / 整周
- 人数选择：1~4 人，菜品数量随人数动态调整
- 食量选择：少食 / 正常 / 大食量，影响每餐菜品数量
- 菜品库覆盖川、粤、鲁、苏、浙、闽、湘、徽八大菜系
- 每餐保证至少 1 菜 + 1 主食 + 1 汤
- 周菜单尽量 7 天不重复，库存不足时自动缩减或重置并提示
- 一键换菜 / 重新生成

## 数据说明

- 所有菜品数据为内置静态清单（108 道菜品 + 45 种主食 + 31 种汤品），非接口获取
- 周不重复约束范围：菜品(dish)、主食(staple)、汤品(soup)均尽量不重复，当库存不足时会自动重置并给出提示，同时保证每餐至少 1 菜 + 1 主食 + 1 汤
...
- 周菜单 dish 去重 / dishReduced / dishReset 标记
- 周菜单全组合（人数×食量）每餐保底 ≥1菜+1主食+1汤
- replaceSingle 换菜
